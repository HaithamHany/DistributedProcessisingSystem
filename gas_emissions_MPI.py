...
def GetNumberOfRows():
    return int(sum(1 for line in open('./datasets/asset_cropland-fires_emissions.csv', encoding="utf8")))
...
def get_emissions_distributed(reading_info: list):
    start_time = time.time()
    df = pd.read_csv('./datasets/asset_cropland-fires_emissions.csv', nrows=reading_info[0], skiprows=range(1, reading_info[1]))
